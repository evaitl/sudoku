...
def add_known(known, unknowns, idx, value):
    ...
    for u in {u for u in unknowns.values() if (u.row==row or u.col==col or u.box==box)}:
        u-={value}
...
def find_unaries(known,unknowns):
    ...
    """
    for u in unknowns:
        s=set.union(u2 for u2 in unknowns.values() if u2.row==u.row and u2.idx != u.idx)
        for v in u:
            if not v in s:
                add_known(known,unknowns,u.idx,v)
                return True
        s=set.union(u2 for u2 in unknowns.values() if u2.col==u.col and u2.idx!=u.idx)
        for v in u:
            if not v in s:
                add_known(known,unknowns,u.idx,v)
                return True
        s=set.union(u2 for u2 in unknowns if u2.box==u.box and u2.idx!=u.idx)
        for v in u:
            if not v in s:
                add_known(known,unknowns,u.idx,v)
                return True
...
def find_rcex(known,unknowns):
    ...
    for u in unknowns:
        for v in u:
            if not any(1 for u2 in unknowns if u2.box!=u.box and u.row==u2.row and u2.contains(v)):
                if elim_values(v,[u2 for u2 in unknowns if u2.box==u.box and u2.row!=u.row]):
                    return True
            if not any(1 for u2 in unknowns if u2.box!=u.box and u.col==u2.col and u2.contains(v)):
                if elim_values(v,[u2 for u2 in unknowns if u2.box==u.box and u2.col!=u.col]):
                    return True
    return False
...
def find_boxex(known,unknowns):
    ...
    for u in unknowns:
        for v in u:
            if not any(1 for u2 in unknowns if u2.box==u.box and u2.row!=u.row and u2.contains(v)):
                if elim_values(v,[u2 for u2 in unknowns if u2.box!=u.box and u2.row==u.row]):
                    return True
            if not any(1 for u2 in unknowns if u2.box==u.box and u2.col!=u.col and u2.contains(v)):
                if elim_values(v,[u2 for u2 in unknowns if u2.box!=u.box and u2.col==u.col]):
                    return True
    return False
...
def find_locked(known,unknowns):
    ...
    """
    for r in range(9):
        rs=[u for u in unknowns if u.row==r]
        for group_size in range(2,len(rs)):
            for group in itertools.combinations(rs,group_size):
                ...
                if len(sug)==group_size:
                    ...
                    if elim_values([v for v in sug], [u for u in unknowns if u.row==r and (not u in group)]):
                        return True
    for c in range(9):
        cs=[u for u in unknowns if u.col==c]
        for group_size in range(2,len(cs)):
            for group in itertools.combinations(cs,group_size):
                ...
                if len(sug)==group_size:
                    ...
                    if elim_values([v for v in sug], [u for u in unknowns if u.col==c and (not u in group)]):
                        return True
    for b in range(9):
        bs=[u for u in unknowns if u.box==r]
        for group_size in range(2,len(bs)):
            for group in itertools.combinations(bs,group_size):
                ...
                if len(sug)==group_size:
                    ...
                    if elim_values([v for v in sug], [u for u in unknowns if u.box==b and (not s in group)]):
                        return True
    return False
...
def solve(puzzle):
    ...
    while run_solvers(solvers,known,unknowns):
        assert all(len(u)>0 for u in unknowns)
        pass
...
def main():
    ...
    parser.add_argument('input_file', help='file with one puzzle per line (81 chars; . or * for empty)')
    parser.add_argument('output_file', nargs='?', help='optional file to write solutions')
